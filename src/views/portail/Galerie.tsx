...
import { X, ZoomIn, ChevronLeft, ChevronRight, Image, Search } from 'lucide-react';
...
export const Galerie: React.FC = () => {
  ...
  const photosFiltrées = photos.filter((p) => {
    ...
    const correspondRecherche = p.titre.toLowerCase().includes(recherche.toLowerCase());
    return correspondCategorie && correspondRecherche;
  });

  const totalPages = Math.ceil(photosFiltrées.length / photosParPage);
  const debutIndex = (pageActive - 1) * photosParPage;
  const photosAffichees = photosFiltrées.slice(debutIndex, debutIndex + photosParPage);
  ...
  return (
    <div className="bg-[#F8FAFC] pb-20 space-y-16 font-outfit">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white rounded-3xl p-5 border border-[#E2E8F0] shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between max-w-4xl mx-auto">
          {/* Input Recherche */}
          <div className="relative w-full md:flex-1">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 rounded-2xl bg-slate-50 border border-[#E2E8F0] focus:outline-none focus:border-[#0B3C91] text-sm"
            />
          </div>
          
          {/* Boutons Catégories (Filter Tabs) */}
          <div className="flex flex-wrap justify-center gap-1.5 w-full md:w-auto">
            {(
              [
                { id: 'toutes', libelle: 'Toutes' },
                { id: 'cultes', libelle: 'Cultes' },
                { id: 'conventions', libelle: 'Conventions' },
                { id: 'missions', libelle: 'Missions' },
                { id: 'speciaux', libelle: 'Événements' },
              ] as const
            ).map((tab) => (
              <button
                key={tab.id}
                ...
                className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all duration-300 ${
                  categorieActive === tab.id
                    ...
                    : 'text-slate-655 hover:text-slate-850 hover:bg-slate-50 border border-transparent hover:border-slate-200'
                }`}
              >
                {tab.libelle}
              </button>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {photosAffichees.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-3xl border border-[#E2E8F0] p-6 space-y-3">
            ...
            <p className="text-slate-500 font-light">Aucune photo disponible.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {photosAffichees.map((photo, index) => {
              const indexGlobal = photosFiltrées.findIndex((p) => p.id === photo.id);
              return (
                <motion.div
                  key={photo.id}
                  layout
                  ...
                  onClick={() => setPhotoAgrandie(indexGlobal)}
      ...
      {/* PAGINATION */}
      {totalPages > 1 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-center items-center gap-2 pt-6">
          <button
            onClick={() => setPageActive((prev) => Math.max(prev - 1, 1))}
            disabled={pageActive === 1}
            className="p-3 rounded-xl border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          <div className="flex items-center gap-1.5">
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => {
              const estProche = Math.abs(page - pageActive) <= 2 || page === 1 || page === totalPages;
              if (!estProche) {
              ...
              return (
                <button
                  ...
                  className={`h-10 min-w-10 px-3 rounded-xl text-xs font-bold transition-all ${
                    pageActive === page
                      ? 'bg-[#0B3C91] text-white shadow-md'
          ...
          <button
            onClick={() => setPageActive((prev) => Math.min(prev + 1, totalPages))}
